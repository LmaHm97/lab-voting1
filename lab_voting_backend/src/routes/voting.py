...
def vote_presentation(presentation_id):
    ...
    # Recompute vote count (race-condition safe)
    presentation.votes = db.session.query(func.count(Vote.id)) \
        .filter(Vote.presentation_id == presentation_id) \
        .scalar()
    db.session.commit()
...
def get_comments(presentation_id):
    ...
    comments = Comment.query.filter_by(presentation_id=presentation_id) \
